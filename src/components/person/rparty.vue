<template>
   ...
   <div><h1 class="main-title badge badge-secondary" style="width: 100%;line-height: 40px;font-size: 30px;">共和党政要名录</h1></div>
							...
							<template v-for="item in items">
								...
								<p >{{item.name}}</p>
													...
													<template v-for="(it,index1) in item.text">
														<template v-if="index1<6&&index1>0">
...
<script>
	...
	export default {
		...
		mounted() {
			...
			//首选加州的数据
			let temparray = require('../../assets/person/json/R.json');
			//检索所有的州
